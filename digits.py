...
class DigitRecognizer(object):
    ...
    def classify(self, img):
        ...
        batch_img = np.zeros((6, 3, self.input_size[0], self.input_size[1]), dtype=np.float32)

        batch_img[0] = _preprocess(img[56:97, 405:434].copy(), width=self.input_size[1], height=self.input_size[0])
        batch_img[1] = _preprocess(img[56:97, 430:458].copy(), width=self.input_size[1], height=self.input_size[0])
        batch_img[2] = _preprocess(img[56:97, 476:504].copy(), width=self.input_size[1], height=self.input_size[0])
        batch_img[3] = _preprocess(img[56:97, 502:529].copy(), width=self.input_size[1], height=self.input_size[0])
        batch_img[4] = _preprocess(img[56:97, 548:576].copy(), width=self.input_size[1], height=self.input_size[0])
        batch_img[5] = _preprocess(img[56:97, 573:603].copy(), width=self.input_size[1], height=self.input_size[0])
        ...
        outputs = np.reshape(trt_outputs[0], (6, 10))
        preds = np.argmax(outputs, axis=1)

        # Time in hh:mm:ss
        # time = str(preds[0]) + str(preds[1]) + ":" + str(preds[2]) + str(preds[3]) + ":" + str(preds[4]) + str(preds[5]) 

        # return seperately
        hh = str(preds[0]) + str(preds[1])
        mm = str(preds[2]) + str(preds[3])
        ss = str(preds[4]) + str(preds[5])
